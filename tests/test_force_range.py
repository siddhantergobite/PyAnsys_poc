...
class ForceRangeTests(unittest.TestCase):
    def test_values_are_evenly_spaced_and_inclusive(self):
        force_range = ForceRange(start_n=100.0, end_n=500.0, increment_n=100.0)
    ...
    def test_equal_legacy_force_has_one_evaluation_point(self):
        force_range = ForceRange(start_n=250.0, end_n=250.0, increment_n=50.0)

        self.assertEqual(force_range.values(), (250.0,))

    def test_invalid_range_is_rejected(self):
        with self.assertRaises(ValueError):
            ForceRange(start_n=500.0, end_n=100.0, increment_n=50.0).validate()

    def test_increment_generates_exact_requested_sequence(self):
        force_range = ForceRange(start_n=100.0, end_n=500.0, increment_n=50.0)
    ...
    def test_increment_includes_end_when_not_an_exact_multiple(self):
        force_range = ForceRange(start_n=100.0, end_n=500.0, increment_n=120.0)

        self.assertEqual(force_range.values(), (100.0, 220.0, 340.0, 460.0, 500.0))
    ...
    def test_api_maps_legacy_single_force_to_a_single_point_range(self):
        ...
        self.assertEqual(request.force_increment_n, 750.0)

    def test_api_rejects_descending_range(self):
        with self.assertRaises(ValidationError):
            SimulationRequest(force_start_n=800.0, force_end_n=100.0, force_increment_n=50.0)

    def test_api_has_no_arbitrary_upper_cap_on_physical_values(self):
        request = SimulationRequest(
            force_start_n=1.0e9,
            force_end_n=2.0e9,
            force_increment_n=1.0e8,
            length_m=10.0,
            width_m=2.0,
            height_m=3.0,
            diameter_m=1.0,
    ...
    def test_api_accepts_increment_for_every_template(self):
        for template in ("cantilever", "corner_bracket", "plate_hole", "pressure_vessel", "table", "bolt", "screw", "nut"):
            request = SimulationRequest(
                template=template,
                force_start_n=100.0,
                force_end_n=500.0,
                force_increment_n=50.0,
            )
            self.assertEqual(request.force_increment_n, 50.0)
